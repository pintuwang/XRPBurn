...
=============================
v12 adds: Retroactive backfill of yesterday's partial entry.
  When today's first run finds yesterday still marked is_partial=True,
  it uses today's midnight_coins (= yesterday's exact EOD coins) to
...
# ── Step 5: backfill yesterday ───────────────────────────────────────────────

def backfill_yesterday(data, today_midnight_coins):
    """
    If yesterday's entry is still is_partial=True, backfill it using
    today's midnight_coins as yesterday's exact end-of-day total_coins.

    Burn:  yesterday.open_coins_xrp − today_midnight_coins  (exact)
    ...
    """
    if today_midnight_coins is None:
        return data
    ...
    if open_coins > today_midnight_coins:
        new_burn = round(open_coins - today_midnight_coins, 6)
        entry["burn_xrp"] = new_burn
        entry["total_coins_xrp"] = today_midnight_coins
        print(f"  Burn: {open_coins:,.4f} − {today_midnight_coins:,.4f} = {new_burn} XRP"
              f"  (was {old_burn})")
    else:
        print(f"  [WARN] open_coins ({open_coins}) <= today_midnight ({today_midnight_coins})"
...
def update_data():
    ...
    print("\n--- Backfill check ---")
    data = backfill_yesterday(data, midnight_coins)
